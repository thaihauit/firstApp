...
class ACFloatingTextfield: UITextField {
    ...
    override func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        return CGRect(x: 4, y: 10, width: bounds.size.width, height: bounds.size.height)
    }
    ...
    private func addFloatingLabel(){
        ...
        labelPlaceholder = UILabel(frame: CGRect(x:  0 , y: 0 , width: self.frame.size.width-5, height: self.frame.height + 5 ))
   ...
   private func floatPlaceHolder(selected:Bool) -> Void {
        ...
        UIView.animate(withDuration: 0.2, animations: {
            ...
            self.labelPlaceholder?.font = UIFont(name: (self.font?.fontName)!, size: 13)
            self.bottomLineView?.frame  =  bottomLineFrame!;
        })
    ...
    private func resignPlaceholder() -> Void {
        ...
        let labelFrame = CGRect(x: 5, y:  0, width: self.frame.size.width-5, height: self.frame.size.height + 5  )
